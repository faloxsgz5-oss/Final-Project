...
export function normalizeReceiptItems(value: unknown): ReceiptItem[] {
  ...
  return value.flatMap((entry) => {
    ...
    const parsedQuantity = optionalNumber(item.quantity ?? item.qty);
...
export function buildReceiptHtml({
  ...
  const itemRows = items.length
    ? items.map((item, index) => {
      ...
      const originalPrice = finalPrice !== null && item.discount !== null
        ? Number((finalPrice + item.discount).toFixed(2))
        : finalPrice;
      return `
      <tr>
        <td class="index">${index + 1}</td>
        <td><strong>${escapeHtml(item.name)}</strong>${item.discount !== null ? `<small class="discount">\u0e2a\u0e48\u0e27\u0e19\u0e25\u0e14 \u0e3f${money(item.discount)}</small><small class="remaining">\u0e23\u0e32\u0e04\u0e32\u0e04\u0e07\u0e40\u0e2b\u0e25\u0e37\u0e2d \u0e3f${money(finalPrice)}</small>` : ""}</td>
        <td class="amount">${item.discount !== null ? `<span class="original">\u0e3f${money(originalPrice)}</span><strong>\u0e3f${money(finalPrice)}</strong>` : `\u0e3f${money(finalPrice)}`}</td>
      </tr>`;
    }).join("")
