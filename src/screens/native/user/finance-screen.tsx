...
const filterPages: Record<Filter, Page> = {all: 'smartlife_finance_day', expense: 'smartlife_finance_expense', income: 'smartlife_finance_income'};
// Finance opens Smart Scan in receipt mode, so a financial document never
// reaches the timetable parser from this entry point.
...
export default function FinanceScreen({onNavigate, page, uid}: Props) {
  ...
  const filter: Filter = page === 'smartlife_finance_income' ? 'income' : page === 'smartlife_finance_expense' ? 'expense' : 'all';
  const load = useCallback(async () => {
    const [pageData, savedBudget] = await Promise.all([
      loadLegacyPageData(uid, `user/${page}`, referenceDate) as Promise<Item>,
      loadMonthlyBudget(uid, currentMonthKey()),
    ]);
    setData(pageData);
    setMonthlyBudget(savedBudget?.amount ?? null);
  }, [page, referenceDate, uid]);
  ...
  const mainLabel = filter === 'income' ? `รายรับ${periodText(periodPage)}` : filter === 'expense' ? `รายจ่าย${periodText(periodPage)}` : 'ยอดคงเหลือ';
  ...
  return <ResponsiveSafeArea style={styles.safe}><View style={styles.screen}><UserGradientBackdrop />
    <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor={C.sage} />} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.header}><View><Text style={styles.eyebrow}>{filter === 'income' ? `รายรับ${periodText(periodPage)}` : filter === 'expense' ? `รายจ่าย${periodText(periodPage)}` : `สรุป${periodText(periodPage)}`}</Text><Text style={styles.title}>การเงิน</Text></View><Pressable accessibilityLabel="สแกนใบเสร็จ" onPress={() => onNavigate(SMART_SCAN_PAGE)} style={styles.receiptButton}><MaterialIcon color="#fff" name="receipt_long" size={21} /></Pressable></View>
      <View style={styles.periodBar}>{periods.map((item) => <Pressable key={item.page} onPress={() => onNavigate(item.page)} style={[styles.periodItem, periodPage === item.page && styles.periodActive]}><Text style={[styles.periodText, periodPage === item.page && styles.periodTextActive]}>{item.label}</Text></Pressable>)}</View>
      ...
      {!data ? <View style={styles.loading}><ActivityIndicator color={C.sage} size="large" /><Text style={styles.loadingText}>กำลังโหลดข้อมูลจาก Firebase</Text></View> : <>
        ...
        {/* Added for monthly budget planning: a direct entry point for a user-set or income-based spending limit. */}
        <Pressable onPress={() => onNavigate('smartlife_monthly_budget')} style={styles.budgetPlanner}><View style={styles.budgetPlannerIcon}><MaterialIcon color="#fff" name="savings" size={20} /></View><View style={{flex: 1}}><Text style={styles.budgetPlannerTitle}>กำหนดงบรายเดือน</Text><Text style={styles.budgetPlannerText}>{monthlyBudget ? `ลิมิตเดือนนี้ ${money(monthlyBudget)}` : 'ตั้งเอง หรือให้ AI แนะนำจากรายรับ'}</Text></View><MaterialIcon color={C.ink} name="chevron_right" size={21} /></Pressable>
        <Pressable onPress={() => onNavigate(filter === 'income' ? 'smartlife_add_income' : SMART_SCAN_PAGE)} style={styles.insight}><View style={styles.insightIcon}><MaterialIcon color="#fff" name={filter === 'income' ? 'north' : 'receipt_long'} size={20} /></View><View style={{flex: 1}}><Text style={styles.insightTitle}>{filter === 'income' ? 'เพิ่มรายรับเอง' : 'สแกนใบเสร็จ'}</Text><Text style={styles.insightText}>{filter === 'income' ? 'บันทึกเงินได้ เช่น งานพิเศษหรือรายได้พิเศษ' : 'ให้ AI แยกหมวดรายจ่ายให้อัตโนมัติ'}</Text></View><MaterialIcon color={C.ink} name="chevron_right" size={21} /></Pressable>
        <View style={styles.sectionHead}><Text style={styles.sectionTitle}>{filter === 'income' ? 'รายการรายรับ' : filter === 'expense' ? 'รายการรายจ่าย' : 'รายการล่าสุด'}</Text><Pressable onPress={() => onNavigate('smartlife_add_income')}><Text style={styles.allLink}>ดูทั้งหมด</Text></Pressable></View>
