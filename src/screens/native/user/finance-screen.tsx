...
// Finance should open the same Smart Scan entry point as the green centre
// action. Receipt-specific scan screens were generated by mistake.
  ...
  return <ResponsiveSafeArea style={styles.safe}><View style={styles.screen}><UserGradientBackdrop />
    <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor={C.sage} />} showsVerticalScrollIndicator={false}>
      ...
      {!data ? <View style={styles.loading}><ActivityIndicator color={C.sage} size="large" /><Text style={styles.loadingText}>กำลังโหลดข้อมูลจาก Firebase</Text></View> : <>
        ...
        <View style={styles.balanceExplain}><MaterialIcon color={C.accent} name="info" size={16} /><Text style={styles.balanceExplainText}>ยอดคงเหลือคำนวณจาก รายรับ {money(income)} - รายจ่าย {money(expense)} = {money(balance)} ในช่วงนี้</Text></View>
        ...
        <Pressable onPress={() => onNavigate('smartlife_line_import')} style={styles.lineImport}><View style={styles.lineImportIcon}><MaterialIcon color="#fff" name="notifications_active" size={20} /></View><View style={{flex: 1}}><Text style={styles.insightTitle}>รับเงินจาก LINE ธนาคาร</Text><Text style={styles.insightText}>เปิดรับแจ้งเตือนครั้งเดียว แล้วตรวจร่างก่อนบันทึกจริง</Text></View><MaterialIcon color={C.ink} name="chevron_right" size={21} /></Pressable>
