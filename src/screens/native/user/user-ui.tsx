...
export function Card({children, style, colors = ['rgba(255,255,255,.98)', '#f8faf5']}: {children: ReactNode; style?: object; colors?: readonly [string, string, ...string[]]}) { return <LinearGradient colors={colors} end={{x: 1, y: 1}} start={{x: 0, y: 0}} style={[styles.card, style]}>{children}</LinearGradient>; }
export function PrimaryButton({label, onPress, disabled = false}: {label: string; onPress: () => void; disabled?: boolean}) { return <Pressable disabled={disabled} onPress={onPress} style={[styles.primary, disabled && styles.disabled]}><LinearGradient colors={['#789a75', '#4d7148']} end={{x: 1, y: 1}} start={{x: 0, y: 0}} style={styles.primaryGradient}><Text style={styles.primaryText}>{label}</Text></LinearGradient></Pressable>; }
export function LoadingBlock({label = 'กำลังโหลดข้อมูลจาก Firebase'}: {label?: string}) { return <View style={styles.loading}><ActivityIndicator color="#668d65" size="large" /><Text style={styles.muted}>{label}</Text></View>; }
export function EmptyBlock({label}: {label: string}) { return <Text style={styles.empty}>{label}</Text>; }

export function UserTabBar({active, onNavigate}: {active?: string; onNavigate: UserNavigate}) {
  const tabs = [
    ['home', 'หน้าหลัก', 'index'], ['calendar_month', 'ตารางเวลา', 'smartlife_calendar_day'], ['add', '', 'smartlife_scan_schedule'], ['account_balance_wallet', 'การเงิน', 'smartlife_finance_day'], ['note_alt', 'โน้ต', 'smartlife_notes'], ['person', 'โปรไฟล์', 'smartlife_profile'],
  ];
...
const styles = StyleSheet.create({
  active: {color: '#4f754b'}, avatar: {alignItems: 'center', backgroundColor: '#e8efe4', borderRadius: 22, height: 44, justifyContent: 'center', width: 44}, avatarText: {color: '#52734b', fontFamily: 'Prompt_700Bold'}, body: {flex: 1, padding: 18}, brand: {color: '#668d65', fontFamily: 'Prompt_600SemiBold', fontSize: 13}, card: {borderColor: 'rgba(255,255,255,.8)', borderRadius: 18, borderWidth: 1, marginTop: 14, padding: 16, shadowColor: '#2c341b', shadowOffset: {height: 8, width: 0}, shadowOpacity: .07, shadowRadius: 18}, disabled: {opacity: .55}, empty: {color: '#879082', fontFamily: 'Prompt_400Regular', fontSize: 13, paddingVertical: 20, textAlign: 'center'}, header: {alignItems: 'center', borderBottomColor: '#e5e9e1', borderBottomWidth: 1, flexDirection: 'row', justifyContent: 'space-between', padding: 18}, loading: {alignItems: 'center', gap: 12, paddingTop: 90}, muted: {color: '#808a7b', fontFamily: 'Prompt_400Regular', fontSize: 12}, plus: {alignItems: 'center', borderColor: '#fff', borderRadius: 31, borderWidth: 5, height: 62, justifyContent: 'center', marginTop: -27, shadowColor: '#23301e', shadowOffset: {height: 8, width: 0}, shadowOpacity: .28, shadowRadius: 13, width: 62}, primary: {borderRadius: 14, marginTop: 15, overflow: 'hidden'}, primaryGradient: {alignItems: 'center', justifyContent: 'center', minHeight: 50}, primaryText: {color: '#fff', fontFamily: 'Prompt_700Bold', fontSize: 15}, safe: {backgroundColor: '#f0f2ec', flex: 1}, scroll: {flexGrow: 1, paddingBottom: 12}, shell: {backgroundColor: '#f0f2ec', flex: 1}, subtitle: {color: '#84907f', fontFamily: 'Prompt_400Regular', fontSize: 12, marginTop: 3}, tab: {alignItems: 'center', justifyContent: 'center'}, tabLabel: {color: '#9aa39a', fontFamily: 'Prompt_500Medium', fontSize: 8, marginTop: 4, textAlign: 'center'}, tabPressed: {opacity: .65, transform: [{translateY: -1}]}, tabs: {alignItems: 'center', borderTopColor: '#e2e6df', borderTopWidth: 1, flexDirection: 'row', height: 78, paddingHorizontal: 0, shadowColor: '#2c341b', shadowOffset: {height: -8, width: 0}, shadowOpacity: .07, shadowRadius: 18}, title: {color: '#29351f', fontFamily: 'Prompt_700Bold', fontSize: 23, marginTop: 2},
});
