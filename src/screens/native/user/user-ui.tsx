...
export function UserTabBar({active, onNavigate}: {active?: string; onNavigate: UserNavigate}) {
  ...
  return <>
    <LinearGradient colors={['rgba(255,255,255,.99)', '#f7f9f4']} end={{x: 1, y: 0}} start={{x: 0, y: 0}} style={styles.tabs}>
      ...
      <Pressable accessibilityLabel="เมนูเพิ่มข้อมูล" onPress={() => setIsBottomSheetOpen(true)} style={({pressed}) => [styles.tab, pressed && styles.tabPressed]}><LinearGradient colors={['#71936e', '#3f633a']} end={{x: 1, y: 1}} start={{x: 0, y: 0}} style={styles.plus}><MaterialIcon color="#fff" name="add" size={31} /></LinearGradient></Pressable>
