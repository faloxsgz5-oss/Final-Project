...
const activityCopy: Record<ActivityKind, {details: string; due: string; location: string; reminder: string; save: string; title: string}> = {
  ...
  task: {details: 'รายละเอียดงาน', due: 'วันที่เริ่มวางแผน', location: 'วิชา / หมวดหมู่', reminder: 'ความสำคัญ', save: 'บันทึกงาน', title: 'เพิ่มงาน'},
...
const suggestions = [
  {location: 'SC1-204', subtitle: 'เหมาะกับช่วงหลังเรียน Digital Systems เพื่อทบทวนก่อนควิซและเตรียมงานส่งวันนี้', time: '16:00 - 17:30', title: 'Data Structures Lab', type: 'activity' as ActivityKind},
  {location: 'หอพัก', subtitle: 'AI เห็นว่ามี Quiz วันศุกร์ จึงแนะนำแบ่งเวลาอ่านสั้น ๆ 45 นาทีนี้', time: '19:30 - 20:15', title: 'อ่าน Quiz DS บทที่ 4', type: 'task' as ActivityKind},
];
...
export default function ActivityFormScreen({page, uid, onNavigate}: {page: FormPage; uid: string; onNavigate: UserNavigate}) {
  ...
  const [isFlexible, setIsFlexible] = useState(form.type !== 'appointment');
  const [allowAiReschedule, setAllowAiReschedule] = useState(form.type !== 'appointment');
  ...
  const save = async () => {
    ...
    const durationMinutes = Math.max(15, Math.min(720, Number(estimatedDuration) || 60));
    const deadline = deadlineDate ? new Date(`${deadlineDate}T23:59:00`) : null;
    if (deadline && Number.isNaN(deadline.getTime())) return Alert.alert('ตรวจสอบวันที่กำหนดส่งอีกครั้ง');
    setSaving(true);
    try {
      const payload = isTransaction
        ...
        : {title, type: activityType, location, color, note, reminder, category, priority, attendees, startAt: startDate.toISOString(), endAt: new Date(startDate.getTime() + durationMinutes * 60_000).toISOString(), deadline: deadline?.toISOString() ?? null, estimatedDurationMinutes: durationMinutes, isFlexible: activityType !== 'appointment' && !attendees.trim() && isFlexible, isLocked: false, allowAiReschedule: activityType !== 'appointment' && !attendees.trim() && isFlexible && allowAiReschedule};
  ...
  const useSuggestion = (suggestion: typeof suggestions[number]) => {
    setActivityType(suggestion.type); setTitle(suggestion.title); setLocation(suggestion.location);
    setTime(suggestion.time.split(' - ')[0]); setNote(suggestion.subtitle); setFormMode('manual');
  };
    ...
    <View style={styles.page}>
      ...
      {formMode === 'ai' ? <AiSuggestions onUse={useSuggestion} /> : <>
        ...
        <View style={styles.formCard}>
          ...
          <View style={styles.twoColumn}><View style={styles.column}><FieldLabel label={copy.due} /><Input onChangeText={setDate} placeholder="เลือกวันที่" value={date} /></View><View style={styles.column}><FieldLabel label={activityType === 'appointment' ? 'ช่วงเวลา' : activityType === 'task' ? 'เวลาเตือน' : 'เวลา'} /><Input onChangeText={setTime} placeholder={activityType === 'appointment' ? 'เลือกช่วงเวลา' : 'เลือกเวลา'} value={time} /></View></View>
          ...
          {activityType === 'task' ? <><FieldLabel label="ความสำคัญ" /><Input icon="notifications_none" onChangeText={setPriority} placeholder="ปกติ" value={priority} /></> : activityType === 'appointment' ? <><FieldLabel label="ผู้เกี่ยวข้อง" /><Input icon="person_outline" onChangeText={setAttendees} placeholder="เพิ่มชื่อเพื่อนหรือกลุ่ม" value={attendees} /></> : <><FieldLabel label="แจ้งเตือน" /><Input icon="notifications_none" onChangeText={setReminder} placeholder="เลือกเวลาแจ้งเตือน" value={reminder} /></>}
          {activityType === 'task' ? <><FieldLabel label="กำหนดส่ง (ไม่บังคับ)" /><Input icon="event" onChangeText={setDeadlineDate} placeholder="YYYY-MM-DD" value={deadlineDate} /></> : null}
          {activityType !== 'appointment' ? <><FieldLabel label="ระยะเวลาที่คาดไว้ (นาที)" /><Input icon="schedule" keyboardType="numeric" onChangeText={setEstimatedDuration} placeholder="60" value={estimatedDuration} /><View style={styles.adaptiveOptions}><AdaptiveToggle active={isFlexible} label="ยืดหยุ่นเวลาได้" onPress={() => setIsFlexible((value) => !value)} /><AdaptiveToggle active={allowAiReschedule && isFlexible} disabled={!isFlexible} label="ให้ AI แนะนำเวลาใหม่" onPress={() => setAllowAiReschedule((value) => !value)} /></View></> : <View style={styles.fixedHint}><MaterialIcon color="#8a6a55" name="lock" size={17} /><Text style={styles.fixedHintText}>นัดหมายเป็นรายการ Fixed และ AI จะไม่ย้ายเวลา</Text></View>}
...
function AiSuggestions({onUse}: {onUse: (suggestion: typeof suggestions[number]) => void}) { return <View style={styles.suggestionArea}><LinearGradient colors={['#6f966f', '#9ab0a0']} end={{x: 1, y: 1}} start={{x: 0, y: 0}} style={styles.suggestionHero}><Text style={styles.suggestionHeroTitle}>พบช่วงว่างที่เหมาะกับกิจกรรม</Text><Text style={styles.suggestionHeroText}>AI ดูจากตารางเรียน งานที่ต้องส่ง และเวลาพัก แล้วเสนอรายการที่ไม่ชนกับคลาสหลัก</Text></LinearGradient>{suggestions.map((suggestion) => <View key={suggestion.title} style={styles.suggestionCard}><Text style={styles.suggestionTitle}>{suggestion.title}</Text><Text style={styles.suggestionText}>{suggestion.subtitle}</Text><View style={styles.tagRow}><Tag label="วันนี้" /><Tag label={suggestion.time} /><Tag label={suggestion.location} /></View><Pressable onPress={() => onUse(suggestion)} style={styles.useSuggestion}><Text style={styles.useSuggestionText}>ใช้กิจกรรมนี้</Text></Pressable></View>)}</View>; }
...
function AdaptiveToggle({active, disabled = false, label, onPress}: {active: boolean; disabled?: boolean; label: string; onPress: () => void}) { return <Pressable disabled={disabled} onPress={onPress} style={[styles.adaptiveToggle, disabled && styles.disabled]}><View style={[styles.toggleDot, active && styles.toggleDotActive]}><MaterialIcon color={active ? '#fff' : '#8b9588'} name={active ? 'check' : 'close'} size={14} /></View><Text style={styles.adaptiveToggleText}>{label}</Text></Pressable>; }
...
const styles = StyleSheet.create({
  adaptiveOptions: {gap: 7, marginTop: 10}, adaptiveToggle: {alignItems: 'center', backgroundColor: '#f1f5ee', borderRadius: 13, flexDirection: 'row', gap: 9, minHeight: 42, paddingHorizontal: 11}, adaptiveToggleText: {color: '#52634f', fontFamily: 'Prompt_600SemiBold', fontSize: 10},
  aiActions: {flexDirection: 'row', gap: 9, marginTop: 12}, aiCard: {backgroundColor: '#edf4eb', borderRadius: 20, marginTop: 16, padding: 15}, aiPrimary: {alignItems: 'center', backgroundColor: '#5f875f', borderRadius: 11, flex: 1, minHeight: 35, justifyContent: 'center'}, aiPrimaryText: {color: '#fff', fontFamily: 'Prompt_700Bold', fontSize: 10}, aiSecondary: {alignItems: 'center', backgroundColor: '#fff', borderRadius: 11, flex: 1, justifyContent: 'center', minHeight: 35}, aiSecondaryText: {color: '#5a7759', fontFamily: 'Prompt_700Bold', fontSize: 10}, aiText: {color: '#70806f', fontFamily: 'Prompt_400Regular', fontSize: 10, lineHeight: 15, marginTop: 3}, aiTitle: {color: '#2e3c2e', fontFamily: 'Prompt_700Bold', fontSize: 12}, close: {alignItems: 'center', backgroundColor: '#fff', borderRadius: 20, height: 40, justifyContent: 'center', width: 40}, color: {borderColor: '#fff', borderRadius: 15, borderWidth: 3, height: 30, width: 30}, colorLabel: {color: '#344235', fontFamily: 'Prompt_700Bold', fontSize: 11, marginTop: 15}, colorRow: {flexDirection: 'row', gap: 7, marginTop: 7}, colorSelected: {borderColor: '#2e3c2e', transform: [{scale: 1.08}]}, column: {flex: 1}, disabled: {opacity: .55}, fixedHint: {alignItems: 'center', backgroundColor: '#f8f1ea', borderRadius: 13, flexDirection: 'row', gap: 8, marginTop: 13, padding: 11}, fixedHintText: {color: '#7c6656', flex: 1, fontFamily: 'Prompt_500Medium', fontSize: 9, lineHeight: 14}, formCard: {backgroundColor: '#fff', borderRadius: 22, boxShadow: '0 8px 20px rgba(42,58,42,.08)', marginTop: 12, padding: 15}, header: {alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between'}, input: {color: '#354133', flex: 1, fontFamily: 'Prompt_500Medium', fontSize: 12, minHeight: 42, paddingHorizontal: 10}, inputShell: {alignItems: 'center', backgroundColor: '#f7f9f5', borderColor: '#e0e7de', borderRadius: 14, borderWidth: 1, flexDirection: 'row', minHeight: 44, paddingHorizontal: 11}, label: {color: '#344235', fontFamily: 'Prompt_700Bold', fontSize: 11, marginBottom: 6, marginTop: 13}, mode: {alignItems: 'center', borderRadius: 14, flex: 1, justifyContent: 'center', minHeight: 39}, modeActive: {backgroundColor: '#5f875f'}, modeText: {color: '#778477', fontFamily: 'Prompt_700Bold', fontSize: 11}, modeTextActive: {color: '#fff'}, modeToggle: {backgroundColor: '#e8eee5', borderRadius: 17, flexDirection: 'row', marginTop: 12, padding: 4}, noteInput: {backgroundColor: '#f7f9f5', borderColor: '#e0e7de', borderRadius: 14, borderWidth: 1, color: '#354133', fontFamily: 'Prompt_400Regular', fontSize: 12, minHeight: 96, padding: 12}, page: {paddingBottom: 6}, save: {alignItems: 'center', justifyContent: 'center', minHeight: 50}, saveShell: {borderRadius: 16, marginTop: 15, overflow: 'hidden'}, saveText: {color: '#fff', fontFamily: 'Prompt_700Bold', fontSize: 14}, suggestionArea: {gap: 10, marginTop: 16}, suggestionCard: {backgroundColor: '#fff', borderRadius: 20, boxShadow: '0 6px 18px rgba(42,58,42,.07)', padding: 14}, suggestionHero: {borderRadius: 20, padding: 16}, suggestionHeroText: {color: 'rgba(255,255,255,.88)', fontFamily: 'Prompt_400Regular', fontSize: 10, lineHeight: 15, marginTop: 4}, suggestionHeroTitle: {color: '#fff', fontFamily: 'Prompt_800ExtraBold', fontSize: 15}, suggestionText: {color: '#657164', fontFamily: 'Prompt_400Regular', fontSize: 10, lineHeight: 15, marginTop: 5}, suggestionTitle: {color: '#314032', fontFamily: 'Prompt_800ExtraBold', fontSize: 14}, tag: {backgroundColor: '#edf2eb', borderRadius: 99, paddingHorizontal: 8, paddingVertical: 4}, tagRow: {flexDirection: 'row', gap: 5, marginTop: 10}, tagText: {color: '#687767', fontFamily: 'Prompt_700Bold', fontSize: 8}, title: {color: '#2f3d2f', fontFamily: 'Prompt_800ExtraBold', fontSize: 22}, toggleDot: {alignItems: 'center', backgroundColor: '#e0e6dd', borderRadius: 9, height: 24, justifyContent: 'center', width: 24}, toggleDotActive: {backgroundColor: '#5f875f'}, twoColumn: {flexDirection: 'row', gap: 9}, typeCard: {alignItems: 'center', backgroundColor: '#fff', borderRadius: 16, flex: 1, gap: 5, minHeight: 61, justifyContent: 'center'}, typeCardActive: {backgroundColor: '#5f875f'}, typeRow: {flexDirection: 'row', gap: 7, marginTop: 12}, typeText: {color: '#778477', fontFamily: 'Prompt_700Bold', fontSize: 9}, typeTextActive: {color: '#fff'}, useSuggestion: {alignItems: 'center', backgroundColor: '#5f875f', borderRadius: 12, justifyContent: 'center', marginTop: 12, minHeight: 37}, useSuggestionText: {color: '#fff', fontFamily: 'Prompt_700Bold', fontSize: 11},
});
