...
function Row({icon = 'circle', title, detail, tone = 'green', side}: {icon?: string; title: string; detail: string; tone?: 'green' | 'purple' | 'rose' | 'red'; side?: string}) { const color = tone === 'red' ? C.red : tone === 'purple' ? '#6572b1' : tone === 'rose' ? '#b98080' : C.sage; const backgroundColor = tone === 'purple' ? '#E8EAF3' : tone === 'rose' ? '#F3E8E8' : tone === 'red' ? C.redSoft : '#f5f8f2'; return <View style={[styles.row, {backgroundColor}]}><View style={[styles.rowIcon, {backgroundColor: `${color}18`}]}><MaterialIcon color={color} name={icon} size={19} /></View><View style={{flex: 1}}><Text numberOfLines={1} style={styles.rowTitle}>{title}</Text><Text numberOfLines={2} style={styles.rowDetail}>{detail}</Text></View>{side ? <Text style={[styles.rowSide, {color}]}>{side}</Text> : null}</View>; }
// Dashboard summary cards follow the same full-width pattern as Monthly Budget and Smart Scan.
...
export default function AdminPortal({onNavigate, onLogout, page, uid}: Props) {
  ...
  const publishAnnouncement = useCallback(async () => {
    ...
    setPublishing(true);
    try {
      await runLegacyDataAction(uid, key, {action: 'create-announcement', payload: {title, message}});
      setAnnouncementTitle('');
      setAnnouncementMessage('');
      await load();
      Alert.alert('สร้างประกาศแล้ว', 'ประกาศถูกบันทึกใน Firebase และพร้อมแสดงแก่ผู้ใช้');
    } catch (error) {
      Alert.alert('สร้างประกาศไม่สำเร็จ', error instanceof Error ? error.message : 'กรุณาลองอีกครั้ง');
    } finally {
      setPublishing(false);
    }
  }, [announcementMessage, announcementTitle, key, load, uid]);

  const body = useMemo(() => {
    if (page === 'admin_dashboard') { const counts = (data.counts ?? {}) as Data; const status = items(data.systemStatus); const scans = items(data.scanLogs); return <><View style={styles.summaryList}><SummaryAction icon="group" label="ผู้ใช้ทั้งหมด" onPress={() => onNavigate('admin_users')} value={counts.users} /><SummaryAction icon="calendar_month" label="ตารางเรียน" onPress={() => onNavigate('admin_ocr_logs')} value={counts.schedules} /><SummaryAction accent="rose" icon="note_alt" label="โน้ตที่บันทึก" onPress={() => onNavigate('admin_categories')} value={counts.notes} /><SummaryAction accent="purple" icon="account_balance_wallet" label="รายการการเงิน" onPress={() => onNavigate('admin_ocr_logs')} value={counts.transactions} /><SummaryAction icon="auto_awesome" label="การใช้งาน AI" onPress={() => onNavigate('admin_ai_knowledge')} value={counts.aiRecommendations} /></View><Pressable onPress={() => onNavigate('admin_system_health')} style={({pressed}) => pressed && styles.pressed}><AdminCard><View style={styles.sectionHead}><Text style={styles.sectionTitle}>System Health</Text><Text style={styles.sectionMeta}>ตรวจสอบล่าสุด</Text></View>{status.length ? status.slice(0, 4).map((item) => <Row detail={`${text(item.detail)} · ${number(item.latencyMs)} ms`} icon="check_circle" key={text(item.id, text(item.name))} side="พร้อม" title={text(item.name)} />) : <Empty label="ยังไม่มีสถานะบริการ" />}</AdminCard></Pressable><Pressable onPress={() => onNavigate('admin_ocr_logs')} style={({pressed}) => pressed && styles.pressed}><AdminCard><View style={styles.sectionHead}><Text style={styles.sectionTitle}>กิจกรรมล่าสุด</Text><Text style={styles.sectionMeta}>{scans.length} รายการ</Text></View>{scans.length ? scans.slice(0, 4).map((item) => <Row detail={`${text(item.extractedText)} · ${date(item.createdAt)}`} icon={item.kind === 'receipt' ? 'receipt_long' : 'calendar_month'} key={text(item.id)} title={item.kind === 'receipt' ? 'OCR ใบเสร็จสำเร็จ' : 'OCR ตารางเรียนสำเร็จ'} tone="purple" />) : <Empty label="ยังไม่มีประวัติการสแกน" />}</AdminCard></Pressable></>; }
    ...
    if (page === 'admin_categories') { const list = items(data.categories); return <AdminCard>{list.length ? list.map((item) => <Row detail={`${text(item.domain)} · ${text(item.labelEn)}`} icon="sell" key={text(item.id)} title={text(item.labelTh)} tone={item.domain === 'note' ? 'rose' : item.domain === 'expense' ? 'purple' : 'green'} />) : <Empty label="ยังไม่มีหมวดหมู่" />}</AdminCard>; }
    if (page === 'admin_announcements') { const list = items(data.announcements); return <><AdminCard style={styles.composer}><View style={styles.composerHead}><View style={styles.composerIcon}><MaterialIcon color={C.sage} name="campaign" size={21} /></View><View style={{flex: 1}}><Text style={styles.sectionTitle}>สร้างประกาศ</Text><Text style={styles.sectionMeta}>ส่งถึงผู้ใช้ทุกคนผ่าน Firebase</Text></View></View><Text style={styles.inputLabel}>หัวข้อประกาศ</Text><TextInput editable={!publishing} onChangeText={setAnnouncementTitle} placeholder="เช่น แจ้งปิดปรับปรุงระบบ" placeholderTextColor="#a6ada3" style={styles.input} value={announcementTitle} /><Text style={styles.inputLabel}>รายละเอียด</Text><TextInput editable={!publishing} multiline onChangeText={setAnnouncementMessage} placeholder="พิมพ์รายละเอียดที่ต้องการสื่อสาร" placeholderTextColor="#a6ada3" style={[styles.input, styles.messageInput]} textAlignVertical="top" value={announcementMessage} /><View style={styles.audience}><MaterialIcon color={C.sage} name="group" size={18} /><Text style={styles.audienceText}>ผู้รับ: ผู้ใช้ทุกคน</Text></View><Pressable disabled={publishing} onPress={publishAnnouncement} style={({pressed}) => [styles.publishButton, (pressed || publishing) && styles.pressed]}>{publishing ? <ActivityIndicator color="#fff" size="small" /> : <MaterialIcon color="#fff" name="send" size={18} />}<Text style={styles.publishText}>{publishing ? 'กำลังบันทึก...' : 'สร้างประกาศถึงทุกคน'}</Text></Pressable></AdminCard><AdminCard>{list.length ? list.map((item) => <Row detail={`${text(item.message)} · ${date(item.createdAt)}`} icon="campaign" key={text(item.id)} title={text(item.title)} tone="purple" />) : <Empty label="ยังไม่มีประกาศ" />}</AdminCard></>; }
  ...
  }, [announcementMessage, announcementTitle, data, logout, onNavigate, page, publishAnnouncement, publishing]);
