...
export default function AuthPortal({mode, onGoogle, onSubmit, onSwitch}: AuthPortalProps) {
  ...
  return <ResponsiveSafeArea style={styles.safe}><KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.screen}>
    ...
    <ScrollView bounces={false} contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      ...
      <LinearGradient colors={['rgba(255,255,255,.96)', 'rgba(248,250,245,.92)']} end={{x: 1, y: 1}} start={{x: 0, y: 0}} style={styles.formCard}>
        ...
        <View style={[styles.socials, registering && styles.socialsRegister]}>
          <Pressable accessibilityLabel="เข้าสู่ระบบด้วย Google" accessibilityRole="button" disabled={busy} onPress={googleLogin} style={({pressed}) => [styles.google, registering && styles.socialHalf, pressed && !busy && styles.pressed, busy && styles.disabled]}>{busy ? <ActivityIndicator color="#4285f4" size="small" /> : <Text style={styles.googleMark}>G</Text>}<Text style={styles.googleText}>{busy ? 'กำลังเชื่อมต่อ...' : 'Google'}</Text></Pressable>
          <Pressable onPress={() => setMessage('Facebook Sign-in จะเปิดใช้งานในขั้นตอนถัดไป')} style={({pressed}) => [styles.facebook, registering && styles.socialHalf, pressed && styles.pressed]}><Text style={styles.facebookMark}>f</Text><Text style={styles.facebookText}>Facebook</Text></Pressable>
        </View>
