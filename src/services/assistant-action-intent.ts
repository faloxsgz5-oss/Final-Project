...
const READ_ONLY_OR_ADVICE_PATTERN =
  /(ควร|แนะนำ|วิเคราะห์|วางแผน|แบ่ง(?:เงิน|งบ|ใช้)?|จัดสรร|ยังไง|อย่างไร|ทำไง|ทำยังไง|เท่าไหร่|เท่าไร|กี่(?:บาท|โมง|งาน|รายการ|วิชา|วัน)?|พอไหม|ดีไหม|ไหม|มั้ย|หรือไม่|หรือเปล่า|ยังทัน|มีอะไร|อะไร|อันไหน|ไหน|เมื่อไหร่|ตอนไหน|ดู|เช็ก|ตรวจ|สรุป|ใกล้.*(?:กำหนดส่ง|เดดไลน์)|(?:กำหนดส่ง|เดดไลน์).*ใกล้|ออม|เก็บ(?:เงิน|ตัง)?|เงินเก็บ|เป้า(?:หมาย)?|ลงทุน|หุ้น|กองทุน|ผลตอบแทน|ดอกเบี้ย|ความเสี่ยง|เงินสำรอง|\?)/i;
