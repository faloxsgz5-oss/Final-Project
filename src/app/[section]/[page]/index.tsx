...
import { getUserRole, registerWithEmail, sendResetEmail, signInWithEmail, signInWithGoogle } from '@/services/auth';
...
function authErrorMessage(error: unknown) {
  ...
  const messages: Record<string, string> = {
    ...
    'auth/operation-not-allowed': 'ยังไม่ได้เปิด Google provider ใน Firebase Authentication',
...
export default function LegacyPageRoute() {
  ...
  if (section === 'login') return <AuthPortal mode={page === 'register' ? 'register' : 'login'} onGoogle={onGoogleAuthenticate} onSubmit={onAuthenticate} onSwitch={(target) => router.push(`/login/${target}` as Href)} />;
  if (!user) {
  ...
  if (section === 'user' && user && ['smartlife_add_activity', 'smartlife_add_task', 'smartlife_add_appointment', 'smartlife_add_income', 'smartlife_save_activity', 'smartlife_save_task', 'smartlife_save_appointment'].includes(page)) return <ActivityFormScreen page={page as 'smartlife_add_activity' | 'smartlife_add_task' | 'smartlife_add_appointment' | 'smartlife_add_income' | 'smartlife_save_activity' | 'smartlife_save_task' | 'smartlife_save_appointment'} uid={user.uid} onNavigate={(target) => router.push(`/user/${target}` as Href)} />;
