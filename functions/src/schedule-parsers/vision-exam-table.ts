...
import {courseCodeKey} from "./vision-course-table";
...
export type ExamTableResult = {
  entries: Map<string, ExamFields>;
  found: boolean;
};
...
function groupLines(words: PositionedWord[]) {
  const lines: {cy: number; words: PositionedWord[]}[] = [];
  ...
  return lines.map((line): PositionedLine => {
...
function textInColumn(words: PositionedWord[]) {
  ...
  if (!/\b(?:19|20|24|25|26)\d{2}\b/.test(value)) return null;
  return value;
}

function headerCenter(words: PositionedWord[], pattern: RegExp) {
  const matches = words.filter((word) => pattern.test(word.text));
  if (matches.length) return matches.reduce((total, word) => total + word.cx, 0) / matches.length;
  ...
  for (const line of lines) {
    for (let windowSize = 2; windowSize <= Math.min(4, line.words.length); windowSize += 1) {
      for (let start = 0; start <= line.words.length - windowSize; start += 1) {
        ...
        if (!pattern.test(window.map((word) => word.text).join(" "))) continue;
...
function pageWidth(annotation: unknown, words: PositionedWord[]) {
  const configured = (annotation as VisionAnnotation | null)?.pages?.[0]?.width;
 ...
 */
export function parseOptionalExamTable(rawText: string, annotation: unknown): ExamTableResult {
  const titleExistsInText = EXAM_TABLE_TITLE.test(rawText);
  if (!titleExistsInText) return {entries: new Map(), found: false};

  const words = positionedWords(annotation);
  const lines = groupLines(words);
  const titleLine = lines.find((line) => EXAM_TABLE_TITLE.test(line.text));
  if (!titleLine || !words.length) return {entries: new Map(), found: true};
  ...
  if (midtermCenter === null && finalCenter === null) {
    ...
    midtermCenter = Math.max(width * .65, (finalCenter as number) - width * .15);
  } else if (finalCenter === null) {
    finalCenter = Math.min(width * .96, midtermCenter + width * .15);
  }

  const columnDistance = Math.max(width * .1, (finalCenter as number) - (midtermCenter as number));
  const inferredMidtermStart = (midtermCenter as number) - columnDistance * .55;
  const midtermStart = groupCenter === null
    ? Math.max(width * .58, inferredMidtermStart)
    : Math.max(width * .58, (groupCenter + (midtermCenter as number)) / 2);
  const finalStart = ((midtermCenter as number) + (finalCenter as number)) / 2;
  const entries = new Map<string, ExamFields>();
...
export function mergeExamFields<T extends StandardScheduleEntry>(entries: T[], examTable: ExamTableResult) {
  return entries.map((entry) => {
    const exam = examTable.entries.get(courseCodeKey(entry.courseCode));
    return {
      ...entry,
