...
export const SCHEDULE_EXTRACTION_SYSTEM_PROMPT = `Act as a high-precision university timetable extraction engine.
...
const requiredTime = {type: "string"} as const;
...
export async function extractScheduleWithGemini(rawText: string, apiKey: string, imageDataUrl?: string) {
  ...
  const response = await fetch("https://generativelanguage.googleapis.com/v1beta/interactions", {
    ...
    body: JSON.stringify({
      ...
      store: false,
      system_instruction: SCHEDULE_EXTRACTION_SYSTEM_PROMPT,
      input,
      response_format: {
        type: "text",
  ...
  if (!outputText) throw new Error("Gemini returned no structured schedule output.");
  const parsed = JSON.parse(outputText) as GeminiScheduleEntry[];
  if (!Array.isArray(parsed)) throw new Error("Gemini returned an invalid schedule payload.");
