...
exports.parseSpatialScheduleGrid = parseSpatialScheduleGrid;
...
function wordsFrom(annotation) {
    const pages = annotation?.pages ?? [];
    return pages.flatMap((page) => (page.blocks ?? []).flatMap((block) => (block.paragraphs ?? []).flatMap((paragraph) => (paragraph.words ?? []).flatMap((word) => { const item = position(word); return item ? [item] : []; }))));
}
...
function windows(line, maximum = 12) {
    const result = [];
...
function courseAnchors(lines, cutoffY) {
    const anchors = [];
    for (const line of lines.filter((item) => item.cy < cutoffY)) {
        for (const candidate of windows(line)) {
            const match = candidate.text.match(COURSE_LINE);
            if (!match)
                continue;
            const courseCode = normal(match[1]).toUpperCase();
            if (anchors.some((item) => item.courseCode === courseCode && Math.abs(item.cx - candidate.cx) < 8 && Math.abs(item.top - candidate.top) < 5))
                continue;
...
function horizontalClusters(line) {
    const clusters = [];
    for (const word of line.words) {
        const current = clusters.at(-1);
        const previous = current?.at(-1);
        const gapLimit = Math.max(12, word.height * 1.8);
        if (!current || !previous || word.left - previous.right > gapLimit)
            clusters.push([word]);
        else
            current.push(word);
    }
...
function parseSpatialScheduleGrid(annotation) {
    const words = wordsFrom(annotation);
    if (!words.length)
        return [];
    const lines = groupLines(words);
    const pageWidth = Math.max(...words.map((word) => word.right));
    ...
    const anchors = courseAnchors(lines, cutoffY);
    const days = dayAnchors(lines, pageWidth, cutoffY);
    return anchors.flatMap((anchor) => {
        const day = [...days].sort((a, b) => Math.abs(a.cy - anchor.cy) - Math.abs(b.cy - anchor.cy))[0];
        if (!day || Math.abs(day.cy - anchor.cy) > Math.max(45, pageWidth * .085))
            return [];
        ...
        const timeCandidates = lines.filter((line) => line.cy > anchor.cy && line.cy <= maxY)
            .flatMap((line) => horizontalClusters(line).flatMap((cluster) => windows({ cy: cluster.cy, words: cluster.words }, 12)
            .map(classTime)
...
//# sourceMappingURL=vision-grid-table.js.map