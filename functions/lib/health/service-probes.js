...
exports.registerSecretForRedaction = registerSecretForRedaction;
exports.redact = redact;
exports.probeGeminiKey = probeGeminiKey;
exports.probeIappOcr = probeIappOcr;
exports.probeVision = probeVision;
exports.probeGoogleCalendar = probeGoogleCalendar;
exports.probeFirebaseAuth = probeFirebaseAuth;
exports.signInProviderHealth = signInProviderHealth;
exports.probeFirebaseStorage = probeFirebaseStorage;
exports.probeFirebaseHosting = probeFirebaseHosting;
exports.probeCloudMessaging = probeCloudMessaging;
exports.appCheckHealth = appCheckHealth;
/**
...
function errorText(error) {
    ...
    if (timedOut)
        return `ไม่ตอบสนองภายใน ${PROBE_TIMEOUT_MS / 1000} วินาที`;
 ...
 */
function httpPhrase(subject, code) {
 ...
 */
async function probeGeminiKey(options) {
    ...
    const result = await measure(() => fetchWithTimeout("https://generativelanguage.googleapis.com/v1beta/models?pageSize=1", { headers: { "x-goog-api-key": apiKey }, method: "GET" }));
    if (!result.ok) {
        return health({
 ...
 */
async function probeIappOcr(apiKey, sortOrder) {
    const id = "iapp-ocr";
    const name = "iApp Thai OCR";
    const configured = Boolean(apiKey.trim());
    const result = await measure(() => fetchWithTimeout("https://api.iapp.co.th/", { method: "GET" }));
    if (!result.ok) {
        return health({
...
async function probeVision(getProjectId, sortOrder) {
    const id = "ocr";
    ...
    const reachable = await measure(() => fetchWithTimeout("https://vision.googleapis.com/$discovery/rest?version=v1", { method: "GET" }));
    if (!reachable.ok) {
        return health({
 ...
 */
async function probeGoogleCalendar(sortOrder) {
    const id = "calendar";
    const name = "Google Calendar Sync";
    const result = await measure(() => fetchWithTimeout("https://www.googleapis.com/discovery/v1/apis/calendar/v3/rest", { method: "GET" }));
    if (!result.ok) {
        return health({
 ...
 */
async function probeFirebaseAuth(sortOrder) {
    const id = "firebase-auth";
    const name = "Firebase Authentication";
    const result = await measure(() => (0, auth_1.getAuth)().listUsers(1000));
    if (!result.ok) {
        return {
            facebookUsers: 0,
            googleUsers: 0,
    ...
    const countProvider = (providerId) => users.filter((user) => user.providerData.some((provider) => provider.providerId === providerId)).length;
    return {
...
function signInProviderHealth(options) {
...
async function probeFirebaseStorage(getMetadata, bucketName, sortOrder) {
...
async function probeFirebaseHosting(projectId, sortOrder) {
 ...
 */
async function probeCloudMessaging(sortOrder) {
    const id = "fcm";
    const name = "Firebase Cloud Messaging";
    const result = await measure(() => (0, messaging_1.getMessaging)().send({
        data: { probe: "health" },
 ...
 */
function appCheckHealth(options) {
...
//# sourceMappingURL=service-probes.js.map