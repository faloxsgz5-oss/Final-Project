...
exports.RECEIPT_EXTRACTION_SYSTEM_PROMPT = `Act as an expert financial NLP engine and OCR receipt parser.
...
function normalizeResult(value, fallbackDate) {
    ...
    const category = CATEGORY_VALUES.includes(payload.category)
        ? payload.category
        : "Others";
    ...
    const items = Array.isArray(payload.items) ? payload.items.flatMap((value) => {
        ...
        if (!name || nonProductText.test(name) || quantityMetadataOnly.test(name) ||
